...
def run_one_job(job: dict, *, adapters=None, loop=None, verbose: bool = False) -> bool:
    ...
    try:
        ...
        if success:
            ...
            # Cron silence suppression — see _is_cron_silence_response.  Replaces
            # the old `SILENT_MARKER in ...upper()` substring check, which both
            # leaked bracketless near-markers ("SILENT" / "NO_REPLY") and wrongly
            ...
            if prev_streak > 0 and job.get("last_failure_notified_at"):
                # Close the loop after a failure streak, even when the agent
                # itself had nothing to deliver — otherwise the operator's last
                # signal was a failure notice. Gated on a notice having actually
                # been delivered: a streak of soft failures the operator never
                # heard about must not produce a "recovered" banner.
                recovery = (
