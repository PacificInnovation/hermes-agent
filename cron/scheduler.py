...
def run_one_job(job: dict, *, adapters=None, loop=None, verbose: bool = False) -> bool:
    ...
    try:
        ...
        # Repeat-failure notification dedup + post-streak recovery bookkeeping.
        # Snapshot the streak BEFORE this run's outcome mutates it in
        # mark_job_run. failure_notified tracks whether a failure notice was
        # actually delivered this run (fed back to mark_job_run for the streak
        # state machine); soft_failure gates it (empty responses never notify).
        ...
        try:
            ...
            # Treat empty/whitespace final_response as a soft failure so
            # last_status is not "ok" — the agent ran but produced nothing
            # useful. Detected HERE (before the delivery/notification decision)
            # so a soft failure neither delivers a blank message nor fires a
            # failure notification. (issue #8585)
            ...
            if success:
                ...
                # Replaces the old `SILENT_MARKER in ...upper()` substring check,
                # which both leaked bracketless near-markers ("SILENT" /
                # "NO_REPLY") and wrongly swallowed a real report that merely
                # quoted "[SILENT]" mid-sentence (#51438, #46917).
                if should_deliver and _is_cron_silence_response(deliver_content):
                    logger.info("Job '%s': agent returned %s — skipping delivery", job["id"], SILENT_MARKER)
                    should_deliver = False
                # Close the loop after a DELIVERED failure streak, even when the
                # agent itself had nothing to deliver — otherwise the operator's
                # last signal was a failure notice. Gated on a notice having
                # actually been delivered (last_failure_notified_at): a streak of
                # soft failures nobody heard about must not produce a "recovered"
                ...
                # First failure of a kind always notifies; identical repeat
                # failures are suppressed until the renotify window elapses. Soft
                # failures never notify (a blank message helps nobody), but the
                # run is still recorded below.
        ...
        # A failure notice that never reached the chat must not open the
        # suppression window — retry the notification next run.
